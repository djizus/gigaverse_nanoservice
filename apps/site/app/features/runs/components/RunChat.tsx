...
export function RunChat({
  ...
  return (
    <div className="card" style={{ height:'100%', display:'flex', flexDirection:'column' }}>
      ...
      {!selectedAgent && (<div style={{ color:'#9ca3af' }}>No agent linked. Use/Create Companion to attach.</div>)}
      {selectedAgent && (
        <>
          <div className="row" style={{ alignItems:'center' }}>
            <div style={{ fontWeight: 600 }}>{selectedAgent.name}</div>
            <div style={{ color:'#9ca3af' }}>{selectedAgent.model} · {selectedAgent.context}</div>
